...
// SON 12 PAÍSES EN TOTAL
const MAX_QUESTIONS = 12;
const INPUT_MODE_START_INDEX = 8; // Empezamos a escribir un poco antes
...
export default function GameSuramerica() {
...
const saveGameResult = async (finalScore: number) => {
  ...
  const prepareRound = (place: Place, index: number, fullList: Place[] = allPlaces) => {
    ...
    if (place.fun_facts && place.fun_facts.length > 0) {
      ...
      setCurrentFact("¡Suramérica es fascinante!");
    }
  ...
  const nextQuestion = () => {
    ...
    // Usamos gameQueue.length en vez de MAX_QUESTIONS por si hay menos datos
    ...
    return (
      <main className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md text-center border-b-8 border-yellow-400">
          ...
          <h1 className="text-3xl font-black text-slate-800 mb-2">¡Reto Suramérica Completado!</h1>
      ...
      <div className="w-full max-w-md flex justify-between items-center mb-2 mt-2">
        <div className="flex flex-col">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
            {isInputMode ? 'NIVEL EXPERTO' : 'Suramérica'}
          </span>

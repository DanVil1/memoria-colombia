...
import { Trophy, RefreshCw, MapPin, Star, Lightbulb, ArrowRightLeft, Keyboard, Send, Timer } from 'lucide-react';
...
export default function Home() {
  ...
  const prepareRound = (place: Place, index: number, fullList: Place[] = allPlaces) => {
    ...
    setIsTimerRunning(true); // Arranca el reloj
  ...
  const handleAnswer = (answer: string) => {
    ...
    setIsTimerRunning(false); // Pausar reloj
      ...
      <div className="w-full max-w-md flex justify-between items-center mb-2 mt-2">
        <div className="flex flex-col">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
            {isInputMode ? 'NIVEL EXPERTO' : 'Calentamiento'}
          </span>
            ...
            {questionType === 'standard' ? (
              <>
                ...
                <p className="text-lg text-slate-500">
                  {isInputMode ? 'Escribe su capital:' : '¿Cuál es su capital?'}
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-lg text-slate-500">
                  {isInputMode ? 'Escribe el departamento:' : '¿A qué departamento pertenece?'}
                </p>
              </>
            )}
          </div>
